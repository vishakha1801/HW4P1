...
class DecoderOnlyTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.dropout = nn.Dropout(dropout) # Final linear layer
        self.norm = nn.LayerNorm(d_model) # Dropout
        self.final_linear = nn.Linear(d_model, num_classes) # Layer norm
    ...
    def forward(self, padded_targets: torch.Tensor, target_lengths: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, dict]:
        ...
        for i, layer in enumerate(self.dec_layers, start=1):
            ...
            running_att[f'layer{i}_dec_self'] = attention
...
class EncoderDecoderTransformer(nn.Module):
    ...
    def encode(self, padded_sources: torch.Tensor, source_lengths: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, dict]:
        ...
        for i, layer in enumerate(self.enc_layers, start=1):
            ...
            x_enc, attention = layer(x_enc, key_padding_mask=pad_mask_src)

            # Save attention weights
            running_att[f'layer{i}_enc_self'] = attention
