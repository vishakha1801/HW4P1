...
class LMTrainer(BaseTrainer):
    ...
    def _train_epoch(self, dataloader) -> Tuple[Dict[str, float], Dict[str, torch.Tensor]]:
        ...
        for i, batch in enumerate(dataloader):
            ...
            with torch.autocast(device_type=self.device.type if isinstance(self.device, torch.device) else self.device, dtype=torch.float16):
                ...
                targets_flat = targets_golden.view(-1)              # Flatten to (batch_size * seq_len)
                raw_loss = self.criterion(raw_preds, targets_flat)
