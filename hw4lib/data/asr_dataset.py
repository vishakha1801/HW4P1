...
class ASRDataset(Dataset):
    ...
    def collate_fn(self, batch) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        batch_feats  = [feat.permute(1,0) for feat, *_ in batch]
        ...
        feat_lengths = torch.LongTensor([feat.size(1) for feat, *_ in batch]) # B
        ...
        if self.partition != "test-clean":
            ...
            batch_shifted = [shift for _, shift, _ in batch] # B x T
            batch_golden  = [gold  for _, _, gold  in batch] # B x T
